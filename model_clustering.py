...
class CAMERA(nn.Module):
    """CAMERA Model: Cross-modal Attention-based Representation Alignment."""
    def __init__(self, config):
        super().__init__()
        self.config = config
        cov = self.config['training']['cov']
        if cov:
            print("Covnet")
    ...
    def to_device(self, device):
        """Move model to device if gpu is used"""
        self.autoencoder.to(device)
        self.mutual_attention.to(device)
        self.projector.to(device)

    def train(self, args, config, train_dataloader, test_dataloader, index_loader, logger, optimizer, device):
        """Training the model.
        ...
        Args:
          ...
          index_loader: index data loader
        ...
        for epoch in range(args.epoch):
            ...
            for iter, (batch_x1, batch_x2, _, _, mask, align, _) in enumerate(train_dataloader):
            ...
            if (epoch + 1) % config['print_num'] == 0:
                ...
                latent_fusion, labels1 = self.both_infer(
                    args, config, test_dataloader, index_loader, device, epoch, index_swap)
                epoch_eval_time = time.time() - time2
                
                scores = evaluation.clustering([latent_fusion], labels1)
                logger.info("\033[2;29m" + 'view_concat ' + str(scores) + "\033[0m")
                print(f"It took {epoch_train_time:.2f}s for epoch training.")
                print(f"It took {epoch_eval_time:.2f}s for evaluation.")

        test_train_time = time.time() - time0
        return scores['kmeans']['accuracy'], scores['kmeans']['NMI'], scores['kmeans']['ARI'], test_train_time

    def both_infer(self, args, config, test_dataloader, index_dataloader, device, epoch, index_swap):
        with torch.no_grad():
            self.autoencoder.eval(), self.mutual_attention.eval(), self.projector.eval()
            fusion_out = []
            class_labels1 = []

            # Get index if needed
            if index_swap is None:
                index_swap = self.get_index(config, index_dataloader)
            ...
            self.autoencoder.train(), self.mutual_attention.train(), self.projector.train()
...
class MutualAttention(nn.Module):
    ...
    def dual_pre(self, s1, s2, index_swap):
        s1, c1, s2, c2 = self.pre(s1, self.c1, s2, self.c2)
